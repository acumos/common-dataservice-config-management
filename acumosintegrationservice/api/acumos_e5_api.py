...
class AcumosE5API:
    ...
    def _get_revisions(self, solution_id: str) -> List:
        ...
        if 'content' not in revision_response:
            raise AcumosE5APIException("Missing required field 'content' in response: " + revision_response)
    ...
    def _request_e5_api(self, endpoint: str) -> dict:
        try:
            ...
            if response.status_code != HTTPStatus.OK.value:
                raise AcumosNetworkException("Error return from Acumos for endpoint: " + endpoint + "."
                                             "Status code: " + response.status_code + " - Content: " + response.text)
            return response.json()
        except ConnectionError as error:
            logging.exception("Exception when connecting to " + self._solution_endpoint + ":" + error)
            raise
    ...
    def validate_connection(self):
        try:
            response = requests.get(self._ping_endpoint, verify=self._verify_certificate,
                                    cert=(self._cert_file, self._private_key_file))
            if response.status_code != HTTPStatus.OK.value:
                raise ConnectionError("Error response from E5: " + response.status_code + " - " + response.text)
        except SSLError as ssl_error:
            raise ConnectionError("Invalid SSL certificates: " + ssl_error)
        except ConnectionError as error:
            raise ConnectionError("Cannot connect to E5 services: " + error)
        except (KeyError, AttributeError) as internal_error:
            raise AcumosE5APIException("Cannot access field/key: " + internal_error)
        except Exception as error:
            raise ConnectionError("Error when connecting to E5 services: " + error)
    ...
    def _download_resource_text(self, solution_id: str, artifact_id: str, file_name: str):
        ...
        response = requests.get(url, stream=True, verify=self._verify_certificate,
                                cert=(self._cert_file, self._private_key_file))
        if response.status_code != HTTPStatus.OK.value:
            raise AcumosE5APIException("Cannot download artifact at " + url)

        logging.debug("Downloaded text from resource: " + file_name)
        r = str(response.text)
        return r
